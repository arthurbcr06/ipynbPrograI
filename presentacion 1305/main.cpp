...
Este programa simula un análisis de colecciones de libros en varias bibliotecas. Lo que hace es:

Crea varias bibliotecas con un nombre y un número total de libros.

Agrega libros en 3 categorías (por ejemplo: Ciencia, Literatura, Historia).

Para cada biblioteca, muestra:

El nombre de la biblioteca

El índice de diversidad (equilibrio entre categorías)

Si cumple con un umbral mínimo de diversidad

Qué categoría tiene más libros
...
int main() {
    try {
        ...
        for (const auto& biblioteca : bibliotecas) { //sirve para recorrer el arreglo de bibliotecas
            std::cout << "Biblioteca: " << biblioteca.getCollectionName() << std::endl;
            
            // Calcular y mostrar ratio de diversidad
            double diversityRatio = biblioteca.calculateCategoryDiversityRatio(); //sirve para calcular el ratio de diversidad
